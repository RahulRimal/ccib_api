"""
Set of pluggable permission classes, filter classes and
...
class PermissionFiltersBase(filters.BaseFilterBackend):
    """
...
class CCIBPermission(permissions.BasePermission):
    """
    ...
    Permission will call action based methods on the model in the following order:
    ...
    If either of the specific permissions do not exist, the Permission will
...
class GlobalPermission(CCIBPermission):
    """
...
class ObjectPermission(CCIBPermission):
    """
...
class PermissionField(fields.Field):
    ...
    This field will call the same developer defined model methods (hence the ) that Permission
    ...
    def bind(self, field_name, parent):
        ...
        assert parent.Meta.model is not None, \
            "Permission is used on '{}' without a model".format(parent.__class__.__name__)
